	...
	with get_rich_pbar(transient=True, auto_refresh=True) as pg:
		...
		for epoch in range(args.epochs):
			...
			if epoch % 20 == 0 and args.start_epochs == 0 and epoch != 0: #epoch 20마다 모델 저장
				if not os.path.exists(f'./parameter/{args.run_name}.pth'):
					os.makedirs(f'./parameter/{args.run_name}.pth')
				torch.save(model.state_dict(), f'./parameter/{args.run_name}.pth')

			for _, (img, gt_score, gt_geo, ignored_map) in enumerate(train_loader):
				img, gt_score, gt_geo, ignored_map = img.to(device), gt_score.to(device), gt_geo.to(device), ignored_map.to(device)
				pred_score, pred_geo = model(img)
				classify_loss , geo_loss, iou_loss = criterion(gt_score, pred_score, gt_geo, pred_geo, ignored_map, args.logger)
				if classify_loss != 0.0:
					pg.print('classify loss is {:.8f}, geo loss is {:.8f}, iou loss is {:.8f}'.format(classify_loss, geo_loss, iou_loss))
			...
			with torch.no_grad():
				...
				for _, (v_img, v_gt_score, v_gt_geo, v_ignored_map) in enumerate(valid_loader):
					...
					v_classifyloss , v_geoloss, v_iouloss = criterion(v_gt_score, v_pred_score, v_gt_geo, v_pred_geo, v_ignored_map, args.logger)
		...
		with torch.no_grad():
			for batch_idx, (t_img, t_gt_score, t_gt_geo, t_ignored_map) in enumerate(test_loader):
				...
				t_classifyloss , t_geoloss, test_iouloss = criterion(t_gt_score, t_pred_score, t_gt_geo, t_pred_geo,t_ignored_map, args.logger)
