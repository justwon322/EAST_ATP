...
class ConcreteDropout(nn.Module):
    ...
    def forward(self, x: Tensor, layer: nn.Module) -> Tensor:
        ...
        dropout_reg *= self.dropout_regulariser * x[0].numel() # x[0].numel() is K
    ...
    def _concrete_dropout(self, x: Tensor) -> Tensor:
        ...
        drop_prob = torch.clamp(input=drop_prob, min=eps, max=1-eps) # plus
        self.p = torch.clamp(input=self.p, min=eps, max=1-eps) # plus
